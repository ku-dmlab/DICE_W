...
import util
from networks import TanhMixtureNormalPolicy, TanhNormalPolicy, ValueNetwork, KValueNetwork
...
class NeuralDICERL(tf.keras.layers.Layer):
    ...
    def __init__(self, observation_spec, action_spec, config):
        ...
        #---------------------------
        self._k_mode = config['k_mode']
        self._k_max = config['k_max']
        self._k_min = config['k_min']
        #---------------------------
        ...
        #---------------------------
        self._k_network = KValueNetwork((observation_spec, action_spec), hidden_sizes=self._hidden_sizes,k_min = self._k_min, k_max = self._k_max, name='k')
        self._k_network.create_variables()
        self._optimizers['k'] = tf.keras.optimizers.Adam(self._lr)
        #---------------------------
        ...
        if self._f == 'chisquare':
            ...
            self._f_k_fn = lambda x, k: tf.where(x < k, x * (tf.math.log(x/k + 1e-10) - 1) + k, 0.5 / k * (x - k) ** 2) # K should be positive
            self._f_k_prime_inv_fn = lambda x, k: tf.where(x < 0, k*tf.math.exp(tf.minimum(x, 0)), k * x + k) # K should be positive
            self._g_k_fn = lambda x,k: tf.where(x < 0, k*tf.math.exp(tf.minimum(x, 0)) * (tf.minimum(x, 0) - 1) + k, 0.5 * k * x ** 2) # K should be positive
            self._r_k_fn = lambda x, k: self._f_k_prime_inv_fn(x, k)
            self._log_r_k_fn = lambda x,k: tf.where(x < 0, x + tf.math.log(k + 1e-10), tf.math.log(k * tf.maximum(x, 0) + k + 1e-10))
        else:
    ...
    def lamb_e_loss(self, e_v, w_e, f_w_e, result={}):
        ...
        return result
    #---------------------------
    def k_loss(self, w, k, result = {}):
        if self._k_mode == 'to_e':
            k_loss = tf.reduce_mean((w - k)**2) + 0.1 * tf.reduce_mean((1 - k)**2)
        else:
            raise NotImplementedError
    ...
    def policy_loss(self, observation, action, w_e, k, result={}):
        # Compute policy loss
        ...
        if self._policy_extraction == 'wbc':
            ...
            if self._gendice_loss_type == 'gendice':
                sampled_log_w_e = self._log_r_k_fn((sampled_e_values - self._lamb_scale * self._lamb_e) / self._alpha, k)
            elif self._gendice_loss_type == 'bestdice':
                sampled_log_w_e = self._log_r_k_fn((sampled_e_values - self._lamb_scale * self._lamb_e) / self._alpha, k)
            else:
                raise NotImplementedError()
    ...
    def train_step(self, initial_observation, observation, action, reward, next_observation, terminal):
        with tf.GradientTape(persistent=True, watch_accessed_variables=False) as tape:
            ...
            #---------------------------
            if self._k_mode == 'to_e':
                tape.watch(self._k_network.variables)
            #---------------------------
            if self._use_policy_entropy_constraint:
            ...
            #---------------------------
            k_values, _ = self._k_network((observation, action))
            #---------------------------
            ...
            w_v = self._r_k_fn(preactivation_v, tf.stop_gradient(k_values))
            f_w_v = self._g_k_fn(preactivation_v, tf.stop_gradient(k_values))
            ...
            w_e = self._r_k_fn(preactivation_e, tf.stop_gradient(k_values))
            f_w_e = self._g_k_fn(preactivation_e, tf.stop_gradient(k_values))
            f_w_e_k = self._g_k_fn(tf.stop_gradient(preactivation_e), k_values)
            ...
            #---------------------------
            loss_result = self.k_loss(w_e, k_values, result=loss_result)
            #---------------------------

            loss_result = self.policy_loss(observation, action, tf.stop_gradient(w_e), tf.stop_gradient(k_values), result=loss_result)
            if self._policy_extraction == 'iproj':
        ...
        k_grads = tape.gradient(loss_result['k_loss'], self._k_network.variables)
        self._optimizers['k'].apply_gradients(zip(k_grads, self._k_network.variables))
...
def run(config):
        ...
        if iteration % config['log_iterations'] == 0:
            ...
            if not int(os.environ.get('DISABLE_STDOUT', 0)):
                ...
                if train_result.get('eval'):
                    ...
                    result_logs_eval.append({'step': iteration, 'eval': train_result['eval']})
    ...
    df = pd.DataFrame.from_dict(result_logs_eval)
    df.to_csv(config['env_name'] + '_' + str(config['alpha']) +'_' + str(config['data_policy_num_mdn_components']) + '_' + str(config['k_max']) + '_' + str(config['k_min']) + '_seed_'+str(config['seed'])+'_'+str(config['lr'])+'.csv', index=False, header=True)
...
if __name__ == "__main__":
    ...
    con = tf.compat.v1.ConfigProto(
        gpu_options=tf.compat.v1.GPUOptions(
            per_process_gpu_memory_fraction=0.22 #최대치의 45%까지
        )
    )
    sess = sess = tf.compat.v1.Session(config=con)
