...
class KValueNetwork(network.Network):

    def __init__(self, input_tensor_spec, hidden_sizes, output_activation_fn=None, last_layer_bias=None, output_dim=None,k_min = 0.5, k_max =2.0, name='K_ValueNetwork'):
        """Create an instance of `ValueNetwork`.
        - hidden_sizes = (256, 256)
        """
        super(KValueNetwork, self).__init__(input_tensor_spec, state_spec=(), name=name)
    ...
    def call(self, inputs, step_type=(), network_state=(), training=False):
        ...
        for layer in self._fc_layers:
            h = layer(h, training=training)
        h = self._last_layer(h)
        h = tf.clip_by_value(h, self._k_min, self._k_max)
